...
class OnDeviceQuestion {
  ...
  Future<QuestionWithChoice> nextQuesionProcess(String questionnaire,
      String currentKey, String choiceYouChoose) async {
    ...
    if (currentKey == null) {
      if (questionnaire != null) {
        if (questionnaire.length > 0) {
          List<Map> mapsQuestion = await database
              .rawQuery(
              "SELECT * FROM $questionTable  WHERE ($questionQuestionnaireId = '${questionnaire
                  .toUpperCase()}') AND ($questionCategory = '-') AND ($questionPosition = 1) ORDER BY $questionPosition ASC")
          ...
          List<Map> mapsChoice = await database
              .rawQuery(
              "SELECT * FROM $choiceTable WHERE ($choiceQuestionId = '${q.id
                  .toLowerCase()}') ORDER BY $questionPosition ASC")
              .then((onValue) => onValue);
      ...
      List<Map> mapsA1 = await database.rawQuery(
          "SELECT * FROM $questionTable  WHERE ($questionId = '${currentKey
              .toUpperCase()}')");
      ...
      if (mapsA1 != null) {
        ...
        if (qA1.type.contains("title")) {
          List<Map> mapsA2 = await database.rawQuery(
              "SELECT * FROM $questionTable  WHERE ($questionCategory = '${qA1
                  .id.toLowerCase()}') ORDER BY $questionPosition ASC");
          ...
          if (mapsA2 != null) {
            if (mapsA2.length > 0) {
              ...
              List<Map> mapsA3 = await database.rawQuery(
                  "SELECT * FROM $choiceTable  WHERE ($choiceQuestionId = '${qA1
                      .id.toLowerCase()}') ORDER BY $choicePosition ASC");
              if (mapsA3 != null) {
              ...
              List<Map> nextQuesRaw = await database.rawQuery(
                  "SELECT * FROM $questionTable  WHERE ($questionQuestionnaireId = '${qA1
                      .questionnaireId
                      .toLowerCase()}') AND ($questionCategory = '-') AND ($questionPosition > ${qA1
                      .position} ) ORDER BY $questionPosition ASC");
              ...
              if (nextQues.type.contains("title")) {
                ...
                List<Map> choiceListRaw = await database.rawQuery(
                    "SELECT * FROM $choiceTable  WHERE ($choiceQuestionId = '${nextQues
                        .id.toLowerCase()}') ORDER BY $choicePosition ASC");
          ...
          if (qA1.category.length > 1) {
            List<Map> questionList = await database.rawQuery(
                "SELECT * FROM $questionTable  WHERE ($questionCategory = '${qA1
                    .category.toLowerCase()}') ORDER BY $questionPosition ASC");

            if (questionList.length == qA1.position) {
              List<Map> questionSetRaw = await database.rawQuery(
                  "SELECT * FROM $questionTable  WHERE ($questionId = '${qA1
                      .category
                      .toUpperCase()}') ORDER BY $questionPosition ASC");
              ...
              List<Map> allQues = await database.rawQuery(
                  "SELECT * FROM $questionTable  WHERE ($questionQuestionnaireId = '${questionSet
                      .questionnaireId
                      .toUpperCase()}') AND ($questionCategory = '-') ORDER BY $questionPosition ASC");
              ...
              if (allQues.length == questionSet.position) {
                ...
                List<Map> nextQuesRaw = await database.rawQuery(
                    "SELECT * FROM $questionTable  WHERE ($questionQuestionnaireId = '${questionSet
                        .questionnaireId
                        .toUpperCase()}') AND ($questionCategory = '-') AND ($questionPosition > ${questionSet
                        .position}) ORDER BY $questionPosition ASC");
                Map nextQuesRaws = nextQuesRaw.first;
                var nextQues = Question.fromMap(nextQuesRaws);
              ...
              List<Map> quesNextRaw = await database.rawQuery(
                  "SELECT * FROM $questionTable  WHERE ($questionQuestionnaireId = '${qA1
                      .questionnaireId
                      .toUpperCase()}') AND ($questionCategory = '${qA1.category
                      .toLowerCase()}') AND ($questionPosition > ${qA1
                      .position}) ORDER BY $questionPosition ASC");
              Map quesNextRaws = quesNextRaw.first;
              var quesNext = Question.fromMap(quesNextRaws);

              List<Map> choiceListRaw = await database.rawQuery(
                  "SELECT * FROM $choiceTable  WHERE ($choiceQuestionId = '${quesNext
                      .category.toLowerCase()}') ORDER BY $choicePosition ASC");
              if (choiceListRaw != null) {
                var choiceList = choiceListRaw.map((model) {
                  var q = Choice.fromMap(model);
                  return q;
            ...
            if (qA1.type.contains("multiply")) {
              if (choiceYouChoose != null) {
                ...
                List<Map> choiceRaw = await database.rawQuery(
                    "SELECT * FROM $choiceTable  WHERE ($choiceId = '${choiceYouChoose
                        .toUpperCase()}') ORDER BY $choicePosition ASC");
                ...
                if (choice.questionId.toLowerCase() == qA1.id.toLowerCase() &&
                  ...
                  List<Map> nextQuesRaw = await database.rawQuery(
                      "SELECT * FROM $questionTable  WHERE ($questionId = '${choice
                          .destinationId
                          .toUpperCase()}') ORDER BY $questionPosition ASC");
                  ...
                  List<Map> nextQuesRaw = await database.rawQuery(
                      "SELECT * FROM $questionTable  WHERE ($questionQuestionnaireId = '${qA1
                          .questionnaireId
                          .toUpperCase()}') AND ($questionCategory = '-') AND ($questionPosition > ${qA1
                          .position}) ORDER BY $questionPosition ASC");
                  Map quesNextRaws = nextQuesRaw.first;
                  var quesNext = Question.fromMap(quesNextRaws);
                ...
                List<Map> nextQuesRaw = await database.rawQuery(
                    "SELECT * FROM $questionTable  WHERE ($questionQuestionnaireId = '${qA1
                        .questionnaireId
                        .toUpperCase()}') AND ($questionPosition > ${qA1
                        .position}) ORDER BY $questionPosition ASC");
  ...
  Future<QuestionWithChoice> nextQuesion(String questionnaire,
      String currentKey, String choiceYouChoose) async {

    QuestionWithChoice nextQues = await nextQuesionProcess(questionnaire, currentKey, choiceYouChoose);

    Database database = await initDatabase();

    if(nextQues.question.category.length > 1){
      List<Map> nextQuesRaw = await database.rawQuery(
          "SELECT * FROM $questionTable  WHERE ($questionId = '${nextQues
              .question.category.toUpperCase()}') ORDER BY $questionPosition ASC");

      Map quesNextRaws = nextQuesRaw.first;
      var quesNext = Question.fromMap(quesNextRaws);
      return QuestionWithChoice(nextQues.question,nextQues.choices,fromQuestion: quesNext);

    }else {
      return QuestionWithChoice(nextQues.question,nextQues.choices,fromQuestion: null);
    }
  ...
  Future<Questionnaire> findQuestionnaire(String questionIds) async {
    ...
    List<Map> maps = await database
        .rawQuery(
        "SELECT * FROM $questionTable WHERE ($questionId = '${questionIds
            .toUpperCase()}')")
        .then((onValue) => onValue);
    Map map = maps.first;
    ...
    return await database
        .rawQuery(
        "SELECT * FROM $questionnaireTable WHERE ($questionnaireId = '${question
